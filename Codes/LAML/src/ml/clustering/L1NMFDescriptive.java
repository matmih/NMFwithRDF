 ...
 */
public class L1NMFDescriptive extends Clustering {
	...
	public L1NMFDescriptive(Options options) {
        ...
        public L1NMFDescriptive(Options options, double _lambda) {
		maxIter = options.maxIter;
		epsilon = options.epsilon;
		gamma = options.gamma;
		mu = options.mu;
	...
	public L1NMFDescriptive(L1NMFOptions L1NMFOptions) {
        ...
        public L1NMFDescriptive(L1NMFOptions L1NMFOptions, double _lambda) {
		maxIter = L1NMFOptions.maxIter;
		epsilon = L1NMFOptions.epsilon;
		gamma = L1NMFOptions.gamma;
		mu = L1NMFOptions.mu;
	...
	public void clustering(Matrix G0) {
		...
		while (true) {
			...
			if (ind % 10 == 0 && verbose) {
				if (calc_OV) {
                                    double err = f1(X,F,G,E_F,E_G);
                                    double normX = Matlab.norm(X,"fro");;
					System.out.println(String.format("Iteration %d, delta G: %f, J: %f, err: %f, norm err: %f", ind, d, J.get(J.size() - 1),err,err/normX));
					// System.out.flush();
				} else {
        ...
        public void clusteringInit(Matrix G0, Matrix F0, Matrix A, Matrix P, int mode) {
		...
		Matrix X = dataMatrix;
                ...
                Matrix PP = P.mtimes(P.transpose());
                Matrix PA = P.mtimes(A.transpose());
		...
		while (true) {
			...
			// Fixing F, updating G
			G = UpdateG(X, G, F, PA, PP, mu, lambda, mode);
			...
			if (calc_OV) {
				J.add(f(X, F, G, E_F, E_G));
                        ...
                        System.out.println("IterationNMF: "+ind);

			if (ind % 10 == 0 && verbose) {
				if (calc_OV) {
                                    double err = f1(X,F,G,E_F,E_G);
                                    double Derr = f1(A,P,G.transpose(),E_F,E_G);
                                    
					System.out.println(String.format("Iteration %d, delta G: %f, J: %f, err: %f, norm err: %f", ind, d, J.get(J.size() - 1),err,err/normX));
                                        System.out.println(String.format("Descriptive error: err: %f, norm err: %f", Derr, Derr/normA));
					// System.out.flush();
				} else {
					System.out.println(String.format("Iteration %d, delta G: %f", ind, d));
					// System.out.flush();
			...
			if (sumAll(isnan(G)) > 0) {
				break;
                                        ...
                                        double err = f1(X,F,G,E_F,E_G);
                                        double Derr = f1(A,P,G.transpose(),E_F,E_G);
                                        System.out.println("Final errors: ");
					System.out.println(String.format("Iteration %d, delta G: %f, J: %f, err: %f, norm err: %f", ind, d, J.get(J.size() - 1),err,err/normX));
                                        System.out.println(String.format("Descriptive error: err: %f, norm err: %f", Derr, Derr/normA));
                
                writeEvalMeasures("resultsL1.txt", ind, maxIter, err, Derr, normX, normA);
	...
	private Matrix UpdateG(Matrix X, Matrix G0, Matrix F, Matrix PA, Matrix PP ,double mu, double lambda, int mode) {

		// min|| X - G * F ||_F^2 + mu * || G ||_1 + lambda * || A - G^t*P ||_F^2
		// s.t. G >= 0
		...
		Vector GSPlusCj = new DenseVector(NExample);
                Vector PP1 = new DenseVector(NExample);
		Vector[] SColumns = Matlab.denseMatrix2DenseColumnVectors(S);
		Vector[] CColumns = Matlab.denseMatrix2DenseColumnVectors(C);
                Vector[] GColumns = Matlab.denseMatrix2DenseColumnVectors(G);
               // Vector[] PPColumns = Matlab.denseMatrix2DenseColumnVectors(PP);
                Vector[] PAColumns = Matlab.denseMatrix2DenseColumnVectors(PaP);
                 Vector[] PPGColumns = Matlab.denseMatrix2DenseColumnVectors(PPG);
		...
		while (true) {
			...
			for (int j = 0; j < K; j++) {
				operate(GSPlusCj, G, SColumns[j]);
                                if(mode == 0){
                                   // operate(PP1,PP,GColumns[j]);
                                    //PP1 = PP1.times(lambda);
                                    PP1 = PPGColumns[j].times(lambda);
                                }
				// GSPlusCj = G.operate(SColumns[j]);
				plusAssign(GSPlusCj, CColumns[j]);
                                if(mode == 0){
                                   // System.out.println("Desc diff: ");
                                    //System.out.println(PP1.get(0)-(PAColumns[j].get(0))*lambda);
                                     plusAssign(GSPlusCj, PP1);
                                     plusAssign(GSPlusCj, PAColumns[j].times(lambda));
                                }
				...
				// G(:, j) = max(G(:, j) - GSPlusC_j, 0)
				for (int i = 0; i < NExample; i++) {
                                    //System.out.println("pr: "+pr[i]);
                                    if((GData[i][j] - pr[i])>0)
					GData[i][j] = Math.max(GData[i][j] - pr[i], 10e-9);
				}*/
			}
		...
		while (true) {
			...
			for (int i = 0; i < K; i++) {
				...
				for (int j = 0; j < NFea; j++) {
					FRow[j] = Math.max(FRow[j] - pr[j], 10e-9);
				}
			}
	...
	private double f(Matrix X, Matrix F, Matrix G, Matrix E_F, Matrix E_G) {
           /* System.out.println("X: "+X.getRowDimension()+" "+X.getColumnDimension());
            System.out.println("F: "+F.getRowDimension()+" "+F.getColumnDimension());
        ...
        private double f1(Matrix X, Matrix F, Matrix G, Matrix E_F, Matrix E_G) {
		double fval = norm(X.minus(G.mtimes(F)), "fro");
		/*return Math.pow(Matlab.norm( X.minus(F.mtimes(G.transpose())), "fro"), 2)
		+ 2 * Matlab.trace( E_F.transpose().mtimes(F) )
		+ 2 * Matlab.trace( E_G.transpose().mtimes(G) );*/
		return fval;
