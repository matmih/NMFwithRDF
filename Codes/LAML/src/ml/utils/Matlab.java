...
public class Matlab {
	...
	public static Matrix getTFIDF(Matrix docTermCountMatrix) {
        ...
        public static Matrix normalizeByRowMax(Matrix A){
            Matrix res = A.copy();
            double max = 0.0;
           ...
           for(int i=0;i<A.getRowDimension();i++){ 
            ...
            for(int j=0;j<A.getColumnDimension();j++)
                res.setEntry(i, j, A.getEntry(i, j)/max);
            ...
            public static Matrix normalizeByColumnMax(Matrix A){
            Matrix res = A.copy();
            double max = 0.0;
           ...
           for(int i=0;i<A.getColumnDimension();i++){ 
            for (int j = 0; j < A.getRowDimension(); j++) {
                            if(j == 0)
                                ...
                                if(A.getEntry(j, i)>max)
                                    max = A.getEntry(j, i);
                            }			
			}
            
            for(int j=0;j<A.getRowDimension();j++)
                res.setEntry(j, i, A.getEntry(j, i)/max);
	...
	public static Matrix rand(int nRow, int nCol) {
		...
		return res;
        	...
        	public static Matrix randP(int nRow, int nCol) {//modified for NMF initialisation
		Random generator = new Random();
		Matrix res = new DenseMatrix(nRow, nCol);
                double num = 0.0;
		for (int i = 0; i < nRow; i++) {
